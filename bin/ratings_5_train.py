...
def train_data(logger,data_master):
	logger.info('******************train model for ratings 1 is started***********************')
	...
	pickle.dump(cv,open('../data/Pos_Feature_CountVectorizer.pickle','wb+'))

	X_train,X_test,y_train,y_test=train_test_split(X_cv,y,test_size=0.25,random_state=42)

	rf=RandomForestClassifier(random_state=18)
	rf.fit(X_train,y_train)
	pickle.dump(rf,open('../data/Pos_Feature_RandomForest.pickle','wb+'))
	print("ratings_5_train execution started")
	...
	pickle.dump(lgbc,open('../data/Pos_Feature_LGBMClassifier.pickle','wb+'))
...
def main():    
	...
	data_master=pd.read_excel('../data/5_train_test.xlsx',index=None)
