...
def add_features(logger,cur,cnx,databasename,sqltable,hostname,dbusername,dbpassword):
	...
	cur.execute(usedb)
	fsql="select * from "+sqltable+" where (UserRating =4 or UserRating=5);"
	cur.execute(fsql)
	...
	cv = pickle.load(open('../data/Pos_Feature_CountVectorizer.pickle','rb'))
	rf = pickle.load(open('../data/Pos_Feature_RandomForest.pickle','rb'))
	lgbc = pickle.load(open('../data/Pos_Feature_LGBMClassifier.pickle','rb'))
	X_cv = cv.transform(X)
	...
	print("ratings_5_predict updated")
