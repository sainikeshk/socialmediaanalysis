...
def add_features(logger,cur,cnx,databasename,sqltable,hostname,dbusername,dbpassword):
	...
	cur.execute(usedb)
	fsql="select * from "+sqltable+" where UserRating =3;"
	cur.execute(fsql)
	...
	cv = pickle.load(open('../data/Rating3_Feature_CountVectorizer.pickle','rb'))
	rf = pickle.load(open('../data/Rating3_Feature_RandomForest.pickle','rb'))
	lgbc = pickle.load(open('../data/Rating3_Feature_LGBMClassifier.pickle','rb'))
	X_cv = cv.transform(X)
	...
	print("ratings_3_predict execution started")
